...
public class FriendService implements InitHandler, ILogin, Dispose {
    ...
    public void del(int playerId, Integer friend) {
        ...
        //删除过的好友
        playerData.getDelFriends().add(friend);

        PlayerData friendPlayerData = playerService.getPlayerData(friend);
        if (friendPlayerData != null) {
            friendPlayerData.getFriends().remove(playerId);
            friendPlayerData.getDelFriends().add(playerId);
        } else {
            ServerLogger.warn("玩家数据不存在，玩家id=" + friend);
        }
    ...
    public Int2Param approve(int playerId, List<Integer> friendIds) {
        ...
        for (int id : friendIds) {
            ...
            //好友活动
            Set<Integer> delFriends = data.getDelFriends();
            if (delFriends == null && delFriends.isEmpty()) {
                continue;
            }
            boolean isDeleteFriend = false;
            for (Integer deletePlayerId : delFriends) {
                if (id == deletePlayerId) {
                    isDeleteFriend = true;
                }
            }

            if (!isDeleteFriend) {
                activityService.tour(playerId, ActivityConsts.ActivityTaskCondType.T_NUMBER_FRIENDS);
                activityService.tour(id, ActivityConsts.ActivityTaskCondType.T_NUMBER_FRIENDS);
            }
        }
