...
public class AttachService implements InitHandler {
    ...
    //复制副本表
    public void copyAttach(SimpleJdbcTemplate mergeDb) {
        StringBuffer sql = new StringBuffer("SELECT * FROM attach");
        List<AttachNew> list = mergeDb.query(sql.toString(), ParameterizedBeanPropertyRowMapper.newInstance(AttachNew.class));
        ServerLogger.warn("副本表开始复制，复制数据=" + list.size());
        for (AttachNew object : list) {
            dao.insertAttach(object);

            Map<Byte, Attach> attachs = new ConcurrentHashMap<Byte, Attach>();
            AttachLogic<?> logic = logicMapping.get(object.getType());
            if (logic == null) {
                continue;
            }
            attachs.put(object.getType(), object.wrap(logic.getAttachClass()));
            attachMapping.put(object.getPlayerId(), attachs);
        }
        ServerLogger.warn("副本表完成复制");
    }
}
